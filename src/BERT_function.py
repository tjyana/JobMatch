...
def match_resume_multilingual(resume_text):
...
def match_resume_minilm(resume_text):
    csv_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../resume-data/jobs.csv'))
    df = pd.read_csv(csv_path)
...
def get_model(match):
    ...
    "Multilingual": 'sentence-transformers/paraphrase-xlm-r-multilingual-v1',
    "cl-tohoku": 'cl-tohoku/bert-base-japanese',
    "sonoisa": 'sonoisa/sentence-bert-base-ja-mean-tokens'
    }
    model = model_dict[match]
    return model
